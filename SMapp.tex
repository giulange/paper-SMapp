...
%In addition, we present three quantitative analysis about soil sealing at three different scales (national, province and municipality).
%Examples of soil sealing metrics are presented, such as an interactive Land Use and Land Cover (LULC) change matrix and the map of rural fragmentation.
...
%Indeed, in terms of transformations towards sealed soils, the class \textit{complex cultivation pattern} resulted the most fragile land use while \textit{forests} resulted the most resilient one.
...
%The study at municipality level --- performed within the province of Napoli --- demonstrated the importance of quantifying a pool of spatial planning indicators towards a better landscape planning.
...
\begin{figure}[t] % FIGURE 02
    ...
    \centerline{\includegraphics[width=500pt]{02_infrastruttura.pdf}}
...
%For example, the client side has a dedicated interface to build the RoI (Figure \ref{fig:SMapp}b, red annotations 14 and 25), which includes different methods such as the multiple selection of several administrative units at any hierarchical level (municipalities, provinces, and regions) or the automatic selection of all the administrative units at the hierarchical level below the one selected (i.e. all the municipalities belonging to a selected province) as carried out in the quantitative analysis at the municipality level (section \ref{sec:caseCOM}). 
...
%and to freely draw a new region of interest directly over the map displayed within the graphical user interface.
%For example, the client side has a dedicated interface to build the RoI (Figure \ref{fig:SMapp}b, red annotations 14 and 25), which includes different methods such as the multiple selection of several administrative units at any hierarchical level (municipalities, provinces, and regions)
...
%It is calculated if the user requests one of the urban sprawl (Table \ref{tab:SMappToolbox}, $ID = 5$) or the residual mean patch surface (Table \ref{tab:SMappToolbox}, $ID = 9$).
...
%The detected object is a urban patch having a unique .
...
For this reason after identification and labelling, urban patches are sorted and then a threshold (e.g. 90\%) is applied to extract only patches below the selected threshold (i.e. belonging to the discontinuous urban fabric). % The threshold was is hard coded in Soil Monitor and was selected in agreement with urban planners.%, even though the parameter could be adjusted by the end user if included in the graphical user interface / accordion pane fo the job submission.
...
%\toberevised{[as results, show the GCI at work on two specific metrics, then describe the case studies ratio (i.e. they are a way to show Soil Monitor at work)]}
...
%Details about their implementation are provided above (sections \ref{sec:mmFragmentation} and \ref{sec:mmDUF}).
...
%\toberevised{[DUF calc]}\\
%An example of the pyramidal architecture based on the CUDA kernels is presented in Figure \ref{fig:ccl}, describing the calculation of the discontinuous urban fabric surface ($S_{DUF}$) using a connected component labelling algorithm specifically developed for Soil Monitor.
...
%}
% WHAT ARE LU and LC:
%Land use/cover is two separate terminologies which are often used interchangeably (Dimyati et al., 1996). Land cover refers to the physical characteristics of earth’s surface, captured in the distribution of vegetation, water, soil and other physical features of the land, including those created solely by human activities e.g., settlements. While land-use refers to the way in which land has been used by humans and their habitat, usually with accent on the functional role of land for economic activities. 
...
\begin{figure}[t] % FIGURE 04
    ...
    %The changes in both the matrix and the map are depicted by colours, each of which is associated with the class in the reference year (1954) consumed by artificial surfaces in the current year (2012)
...
%Although the first version of the implementation was carried out without great involvement of stakeholders, because of money, time, and manpower constraints, at least two middle phases of consultation managed to apply modifications to the Soil Monitor design and functionality. 
...
%This required the implementation of stand-alone CUDA-C codes \toberevised{from scratch} tailored to a GPU-based geospatial processing.
...
%The limitation of developing new code \toberevised{from scratch} is overcome by high degree of code reusability, lower power consumption and higher computing scalability of GPU technologies with respect to standard CPU approaches.
...
\section{Conclusions}
% 1) summarize the main points of the manuscript
% (a) SS is an important issue, (b) need tools to help urban planning
...
% Therefore, authors stress on the current strong gap in developing and providing integrated operational tools dealing with soil sealing to support quantitative analysis, awareness building and decision making by stakeholders.
...
%
% 2) restate the thesis with different words
%A viable solution to fill this gap fits with the general aim of our work, which deals with the development of a new type of geospatial decision support system --- a web-based decision support system, namely Soil Monitor --- developed on top of Geospatial Cyber-Infrastructures (GCI) and providing a multidisciplinary operational tool useful for a multi-stakeholder community to challenge soil sealing at different spatial scales.
...
%
% 3) make a final comment about the paper main idea
...
%
% 4) emphasise an action that I'd like the reader to take
...
%\toberevised{Indeed, this work shows that %starting from the problem statement in a specific research domain, 
%it is feasible to combine recent achievements in terms of data, metrics, WebGIS and high performance computing technologies to build a geospatial cyberinfrastructure.
%This kind of technology demonstrated promising capability to quantitatively measure soil sealing in order to give a contribution to mitigate continual phenomenon worldwide}. 
